#!/usr/bin/env python3
"""
Graph to Omega Workflow using Direct QCI Client API

This script implements a complete workflow for computing maximum clique size (omega)
from both DIMACS and QPLIB JSON files using direct submission to Dirac-3 via qci_client API.

Workflow: DIMACS/QPLIB → QCI Polynomial File → Dirac-3 Job → Energy Results → Omega

Usage:
    python scripts/graph_to_omega.py input.dimacs [OPTIONS]
    python scripts/graph_to_omega.py input.json [OPTIONS]
...
def submit_to_dirac(
    ...
    job_name: str = "graph_omega_computation",
    num_samples: int = 100,
    ...
    try:
        ...
        job_body = client.build_job_body(
            ...
            job_name=job_name,
            job_tags=job_tags or ['graph', 'omega_computation'],
...
# =============================================================================
# DIMACS INTEGRATION FUNCTIONS  
# =============================================================================

def dimacs_to_qplib_file(dimacs_file: str) -> str:
    """
    Convert DIMACS file to QPLIB JSON file.
    
    Args:
        dimacs_file: Path to DIMACS format file
        
    Returns:
        Path to created QPLIB JSON file
    ...
    try:
        # Convert DIMACS to QPLIB
        qplib_data = dimacs_to_qplib_func(dimacs_file)
        
        # Create output file path
        dimacs_path = Path(dimacs_file)
        qplib_file = dimacs_path.with_suffix('.qplib.json')
        
        # Save QPLIB data
        with open(qplib_file, 'w') as f:
            json.dump(qplib_data, f, indent=2)
        
        print(f"✅ Converted {dimacs_file} → {qplib_file}")
        return str(qplib_file)
        
    except Exception as e:
        raise ValueError(f"Error converting DIMACS to QPLIB: {e}")
...
def compute_omega_from_file(
    ...
    """
    Complete workflow: Load file → Submit to Dirac → Compute omega.
...
Examples:
  ...
  python scripts/graph_to_omega.py input.dimacs --solution-precision 0.001
    ...
    print("🎯 Graph to Omega Computation via Direct QCI Client")
    print("=" * 60)
    
    try:
        # Run complete workflow
        omega, best_energy, all_energies, best_solution = compute_omega_from_file(
        ...
        if args.format == "json":
            result = {
                ...
                "workflow": "Graph->QCI->Dirac-3->Omega"
            }
            ...
            print(f"Workflow: Graph → QCI Client → Dirac-3 → Omega")
            print(f"{'='*60}")
