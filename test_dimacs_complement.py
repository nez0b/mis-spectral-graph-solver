#!/usr/bin/env python3
"""
C125.9 Complement Graph MIS Analysis (Version 2)

Exact copy of examples/test_er.py but with C125.9 complement graph input.
This finds the maximum clique of C125.9 by solving MIS on its complement,
using the mathematical identity: Max clique in G = MIS in complement(G).

No MIS-to-clique conversion logic - just applies existing MIS algorithms
to the complement graph directly. The result will be the maximum independent
set of the complement, which equals the maximum clique of the original.

Graph transformation: C125.9 (6963 edges, dense) → complement (787 edges, sparse)
"""
...
def load_dimacs_complement_graph(graph_name):
    ...
    # Load dimacs graph
    # original_graph = read_dimacs_graph("DIMACS/C125.9.clq")
    # graph_name = "keller4"
    original_graph = read_dimacs_graph(f"DIMACS/{graph_name}.clq")

    print(f"Original {graph_name} properties:")
    print(f"  Nodes: {original_graph.number_of_nodes()}")
    print(f"  Edges: {original_graph.number_of_edges()}")
    print(f"  Density: {original_graph.number_of_edges() / (original_graph.number_of_nodes() * (original_graph.number_of_nodes() - 1) / 2):.3f}")
    print(f"  Average degree: {2 * original_graph.number_of_edges() / original_graph.number_of_nodes():.2f}")
    
    # Compute complement
    complement_graph = nx.complement(original_graph)
    
    print(f"\nComplement graph properties:")
    print(f"  Nodes: {complement_graph.number_of_nodes()}")
    print(f"  Edges: {complement_graph.number_of_edges()}")
    print(f"  Density: {complement_graph.number_of_edges() / (complement_graph.number_of_nodes() * (complement_graph.number_of_nodes() - 1) / 2):.3f}")
    print(f"  Average degree: {2 * complement_graph.number_of_edges() / complement_graph.number_of_nodes():.2f}")
    
    # Validation
    max_edges = original_graph.number_of_nodes() * (original_graph.number_of_nodes() - 1) // 2
    total_edges = original_graph.number_of_edges() + complement_graph.number_of_edges()
    print(f"\nValidation: {original_graph.number_of_edges()} + {complement_graph.number_of_edges()} = {total_edges} (expected: {max_edges}) ✅" if total_edges == max_edges else "❌")
    
    print(f"\nStrategy: Solve MIS on sparse complement ({complement_graph.number_of_edges()} edges) instead of max clique on dense original ({original_graph.number_of_edges()} edges)")
...
def run_comprehensive_comparison(G):
    ...
    # Configure benchmark (adjusted for complement graph analysis)
    benchmark_config = {
        'num_random_runs': 10,  
        'fast_timeout': 30000.0,    # Extended timeouts for 125-node complement
        'medium_timeout': 30000.0,
        'slow_timeout': 30000.0,
        'jax_config': {
            'learning_rate_pgd': 0.02,
            'learning_rate_md': 0.01,
            'max_iterations': 500,   # Moderate iterations for complement
            'num_restarts': 5,       # Moderate restarts
            'tolerance': 1e-6,
    ...
    results = run_algorithm_comparison(
        G, 
        f"dimacs_complement_{num_nodes}_nodes",
...
def analyze_results(results):
    ...
    print("="*60)
    print("NOTE: Results show MIS of dimacs complement = Max clique of dimacs original")
    ...
    print(f"\nOptimal independent set size (= max clique size in original): {optimal_size}")
...
def visualize_graph_and_solutions(G, results, optimal_size, output_dir, graph_name):
    ...
    pos = nx.circular_layout(G)  # Use circular for large graph
    ...
    for i, (alg, result) in enumerate(successful_results.items()):
        ...
        # Node sizes: smaller for large graph, larger for MIS nodes
        node_sizes = [100 if node in result.independent_set else 30 for node in G.nodes()]
        
        # Draw graph - simplified for 125 nodes
        nx.draw_networkx_nodes(G, pos, node_color=node_colors, 
                              node_size=node_sizes, ax=ax)
        nx.draw_networkx_edges(G, pos, alpha=0.2, width=0.3, ax=ax)
        # No labels for 125-node graph
        
        # Title with results
        ratio = result.set_size / optimal_size
        title = f"{alg}\nMIS Size: {result.set_size}/{optimal_size} (ratio: {ratio:.3f})"
    ...
    plt.suptitle(f'MIS Solutions on {graph_name} Complement\n(= Max Clique Solutions for {graph_name} Original)\nComplement: {G.number_of_nodes()} nodes, {G.number_of_edges()} edges', fontsize=14)
    plt.tight_layout()
    plt.savefig(output_dir / f'{graph_name}_complement_mis_solutions.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_convergence_histories(results, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'c125_jax_convergence_analysis.png', dpi=300, bbox_inches='tight')
    plt.close()


def plot_algorithm_comparison(analysis_data, output_dir, graph_name):
    ...
    ax1.set_title('MIS Size Comparison\n(= Max Clique Size in C125.9 Original)')
    ax1.set_ylabel('MIS Set Size')
    ...
    plt.tight_layout()
    plt.savefig(output_dir / f'{graph_name}_algorithm_comparison.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.title(f'Quality vs Speed Trade-off for {graph_name} Complement MIS\n(Bubble size ∝ Oracle calls)')
    plt.grid(True, alpha=0.3)
    plt.axhline(y=1.0, color='red', linestyle='--', alpha=0.7, label='Optimal')
    plt.legend()
    plt.savefig(output_dir / f'{graph_name}_timing_analysis.png', dpi=300, bbox_inches='tight')
    plt.close()


def generate_summary_report(G, results, analysis_data, optimal_size, output_dir, graph_name):
    """Generate a comprehensive summary report."""
    print("\nGenerating summary report...")
    
    report_path = output_dir / f"{graph_name}_complement_report.txt"
    
    with open(report_path, 'w') as f:
        f.write(f"{graph_name} Maximum Clique Analysis via Complement Graph MIS\n")
        f.write("="*60 + "\n\n")
        
        f.write(f"Strategy: Find max clique in {graph_name} by solving MIS on its complement\n")
        f.write(f"Mathematical identity: Max clique in G = MIS in complement(G)\n\n")
        
        f.write(f"Complement Graph: {G.number_of_nodes()} nodes, {G.number_of_edges()} edges\n")
        f.write(f"  Density: {G.number_of_edges() / (G.number_of_nodes() * (G.number_of_nodes() - 1) / 2):.3f}\n")
        f.write(f"  Maximum clique size in original {graph_name}: {optimal_size}\n\n")
        ...
        for alg, data in analysis_data.items():
            f.write(f"\n{alg}:\n")
            f.write(f"  MIS size: {data['set_size']}/{optimal_size} (ratio: {data['ratio']:.3f})\n")
            f.write(f"  Runtime: {data['runtime']:.4f} seconds\n")
            f.write(f"  Oracle calls: {data['oracle_calls']}\n")
            f.write(f"  MIS nodes (= clique in original): {sorted(data['independent_set'])}\n")
        
        # Key insights
        f.write(f"\nKey Insights:\n")
        f.write(f"- Complement strategy: Solve MIS on sparse graph (787 edges) vs clique on dense (6963 edges)\n")
        f.write(f"- Exact methods found maximum clique of size {optimal_size} in {graph_name}\n")
        ...
        f.write(f"- Theoretical validation: Original + complement edges sum correctly\n")
...
def main(graph_name):
    """Run dimacs complement graph MIS analysis (= max clique in original)."""
    print(f"{graph_name} Maximum Clique Analysis via Complement Graph MIS")
    print("Mathematical foundation: Max clique in G = MIS in complement(G)")
    print("Strategy: Solve MIS on sparse complement instead of clique on dense original")
    ...
    # Load complement graph
    G = load_dimacs_complement_graph(graph_name)
    ...
    if analysis_data:
        ...
        visualize_graph_and_solutions(G, results, optimal_size, output_dir, graph_name)
        plot_convergence_histories(results, output_dir)
        plot_algorithm_comparison(analysis_data, output_dir, graph_name)
        
        # Generate report
        generate_summary_report(G, results, analysis_data, optimal_size, output_dir, graph_name)
    
    total_time = time.time() - start_time
    print(f"\n{graph_name} complement analysis completed in {total_time:.1f} seconds")
    print(f"Maximum clique size in original {graph_name}: {optimal_size}")
